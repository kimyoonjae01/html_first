...
* ??
### semantic tag란?
* 맨틱 태그 (Semantic Tag)는 포함된 콘텐츠의 특정 의미를 정의하고 목적을 갖는 태그입니다.
...
* 홈페이지 상단 부분 얘기하는거같음 
### nav
* 로고 및 웹 사이트 주요 네비게이션을 묶어주는 웹 사이트 레이아웃태그
* 다른 페이지로의 링크를 보여주는 구획을 나타냄, 자주 쓰이는 예제는 메뉴,목차,색인 입니다.
### gnb,lnb, snb
gnb - 글로벌 네비게이션바 - nav 묶이는 대상
lnb - 로컬 네비게이션바 - 
snb - 서브 네비게이션 바 -
## ul, ol, li
* ol은 li를 넣을 때 순서가 상관 있어야 할 때 ol 삽입
* ul은 순서가 상관 없을 때 적는것
## details, summary
* “열림”상태일 때만 내부 정보를 보여주는 정보 공개 위젯을 생성합니다.
* summary를 넣으면 위젯 옆에 제목이 나옵니다
## footer, mark, main, time
* footer 는 홈페이지 맨 하단에 전화번호,주소 등을 기입하는 곳을 얘기한다. 
* mark 는 현재 맥락에 관련이 깊거나 중요해 표시 또는 하이라이트한 부분을 나타냅니다.
* main은 전체 내용에 해당하는 걸 얘기합니다 . / header와 footer안에 들어가는 모든 내용
* time은 검색엔진에 검색 시 언제 이 글을 올렸나, 이 글을 수정했나 나오는 것을 얘기합니다.